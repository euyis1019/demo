...
from agent_world.drama_demo.shared.prompt_paths import stories_root

_DRAMA_ROOT = stories_root().resolve().parent.parent  # config/stories → config → drama_demo
# 绝对禁止写入的目录（运行期存档 + 包源码）。
_FORBIDDEN_ROOTS = tuple(
    (_DRAMA_ROOT / name).resolve()
