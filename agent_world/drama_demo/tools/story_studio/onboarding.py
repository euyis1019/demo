...
ONBOARDING_SCHEMA: Dict[str, Any] = {
    ...
    "type": "object",
    "required": ["title", "background", "tips", "hook"],
    "properties": {
        # 故事标题/一句话定位
        "title": {"type": "string"},
        ...
        # 一句「开场钩子/线索」：勾起好奇、暗示一个该去探的方向（指向开局可触发的动作），不剧透。
        # 删「分幕/任务」后用它给玩家随时可见的目标感（前端常驻「线索」位）。
        "hook": {"type": "string", "minLength": 6},
    },
}
...
只输出 JSON：
{
  ...
  "hook": "一句『开场钩子/线索』——勾起玩家好奇、暗示一个此刻就该去探的方向，常驻在界面上给玩家目标感"
}
hook 是玩家随时可见的「线索」(删了分幕/任务后用它补目标感)：一句话、有画面感、像旁白对玩家的耳语，
**指向开局就能触发的那个动作/疑点**（若给了开局可触发动作列表，hook 要扣住其中一个，让玩家照着它走就有反应），
但只勾不破——别点名真凶、别剧透结局、别写成"请去问X"式的生硬任务。例：「阿芸的目光总飘向楼上那扇锁死的门——那把钥匙真像她说的丢了吗？」
