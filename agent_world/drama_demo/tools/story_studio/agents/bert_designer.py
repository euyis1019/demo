  ...
  "berts": [
    {
      ...
      "hint": "给玩家看的『线索』：一句勾而不破的下一步提示，指向上面这条 trigger（让玩家知道此刻可以朝哪使劲），但别剧透反应/真相、别写成生硬任务。例：『赵老板的目光总往楼上飘——那把"丢了"的钥匙，真丢了吗？』",
...
- **每条非结局 bert 都配一句 `hint`（玩家向线索）**：勾而不破地指向它的 trigger，让玩家知道此刻能朝哪使劲，
  但别剧透反应/真相、别写成"请去问X"式的生硬任务。这些 hint 会随玩家触发 bert **自动更新成下一条线索**，
  是玩家删幕后的推进指引——所以要写得像耳边低语的悬念钩子、且和它的 trigger 对得上（玩家照 hint 做就能触发该 bert）。
- **结局**：至少 1 条 ending 非空的 bert；故事抉择空间大就多给几个（good/neutral/bad 自由搭，不必凑齐）。
  结局 bert 只要 trigger + ending + **hint**（不需要 target/reaction）。
  ★**结局必须串在反应链末端，不能开局即触发**：每条结局 bert 都要么有**非空 requires**（写明要先触发过哪些
  前置 bert），要么被某条非开局 bert 用 arms 指向。**绝不让结局 bert 的 requires 为空**——否则玩家第一句就
  可能被判中结局、故事没玩就收场。先经历前置反应链，才走得到任何结局。
  ★**每条结局也要配一句 `hint`（收场指引）**：告诉玩家『此刻可以怎样给故事画句号』（如『你已掌握真相，可以
  当面揭发他讨回公道，或放他一马私下了结』），勾住收场动作、别剧透后果。这句 hint 会在该结局**可触发时**亮给
  玩家，是玩家「知道故事能在这结束、且怎么结束」的关键。
  ★**结局要能从「主线」走到，别把所有结局都锁在某个配角支线后面**：让玩家把**主角/核心揭秘链**走到自然终点后，
  就有**至少一个结局触手可及**（其 requires 基本只依赖这条主线）；别要求玩家必须把每个配角的支线都跑完才有任何
  结局可收——否则玩家会卡在「主线高潮已过（真相已揭、主角已坦白），却没有任何结局能收场」，体验极差。配角支线可以
  解锁**额外/更好**的结局，但不该是**唯一**的收场路径。
- **bert 数量由剧情自然决定**（简单冲突 3-5 条，丰满的 8-15 条都行），环环相扣、不注水。
- 每条非结局 bert 都必须能从「开局就上膛的 bert」经 arms 链到达，别留永不触发的孤儿。
- **开局可玩面（重要，否则玩家一上来无从下手）**：开局就上膛的 bert ＝ requires 为空、且**没有**任何别的 bert
  用 arms 指向它。务必让开局上膛的非结局 bert **至少有 2 条**，并尽量**覆盖多个在场 NPC**——
  玩家无论先跟谁搭话、先问哪条线索，都该有角色能接住、有反应；别把所有开局入口都堆在同一个人/同一件事上。
- **requires 与 arms 不能打架**：一条 bert 要「开局就能触发」，就**别**让任何别的 bert 用 arms 指向它
  （被 arms 指向＝必须等那条先触发）；要把它当「后续」串在链上，就给它**非空 requires** 写明前置。
  绝不出现「requires 为空却又被别的 bert arms 指向」的自相矛盾——那会让你以为开局能触发、实际却被锁死。
