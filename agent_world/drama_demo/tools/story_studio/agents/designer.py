...
- **结局数由你按这个故事的抉择空间自己定**：基本是线性、收束单一的故事给 1-2 个；有重大对立/道德抉择/多方
  博弈的故事给 3-5 个（good/neutral/bad 自由搭配，不必凑齐三种、也别为凑数硬加）。
- 让**最后一个任务**（收官任务）分叉到这些结局：玩家用不同方式完成/搞砸收官任务，分别导向不同结局。
  即：最后一个任务 node 有多条出边，各指向一个结局；其余任务都只有 1 条出边指向下一个任务。

★**任务数也由你按剧情自然决定**：这个故事讲完需要几个环节就给几个（短故事 2-3 个，丰满的故事 5-8 个甚至更多
都行），只要环环相扣、不注水、不为凑数硬加。**仅当 brief 明确给了 target_tasks 时**才优先按它，否则你自己定。

硬性约束（只保证「可玩/合法」，不限制你的设计自由）：
- initial_node 必须在 nodes 中（它是第一个任务）；任务 id 与结局 id 不重叠。
- 任务之间不得成环；每个结局都要能从第一个任务沿边抵达；不得有够不着的任务或结局。
- 边的起点必须是任务（结局是终结的，不可作起点）。
- 至少 1 个任务、1 个结局（数量上不再有其它限制——多少由剧情定）。
