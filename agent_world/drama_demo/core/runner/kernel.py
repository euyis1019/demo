...
@dataclass
class DramaKernel(MinimalKernel):
...
def _load_dotenv_into_environ() -> None:
    candidates = (
        _DRAMA_DEMO_ROOT / ".env",
        _DRAMA_DEMO_ROOT.parent / "demo" / ".env",
    )
...
async def build_kernel(
    ...
    *,
    ...
    dispatcher = DramaActionDispatcher(
        world_state=world_state,
        f2f_bus=f2f_bus,
        rdc_bus=rdc_bus,
        grp_bus=grp_bus,
    ...
    for a in scenario.get("agents", []):
        aid = int(a["agent_id"])
        from agent_world.drama_demo.core.runner.integration import virtual_player

        if virtual_player.is_f08_enabled() and aid == int(virtual_player.player_agent_id()):
            continue
        agent = DramaAgent(
            agent_id=aid,
    ...
    step_cls = DramaWorldStep if parallel_agents else WorldStep
