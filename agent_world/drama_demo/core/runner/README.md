...
外的文件**不得**直接 `import agent_world.drama_demo.features.*`。

## 文件职责

| 文件 | 作用 |
|------|------|
| `run_drama.py` | 进程入口：解析 `--config/--sim-dir`、`build_kernel`、起 `WorldLoopOrchestrator` + IPCServer、wire IPC handlers、写 env_status（starting/running/stopped）。 |
...
| `drama_agent.py` | HBM Agent：组装 system prompt（soul + story knowledge + thread recap + conversation hints）、调 LLM、产出动作；含 `story_advance` 工具定义（approve_visitor/expel_ceos/offer_join/offer_seed…）。 |
| `drama_dispatcher.py` | HBM 动作分发：把 Agent 工具调用路由到引擎总线；静默忽略 `request_move` 等。 |
