...
from agent_world.drama_demo.features.f01_session.paths import get_world_db_path
from agent_world.drama_demo.shared.errors import DatabaseReadError
from agent_world.drama_demo.shared.settings import DB_CONNECT_TIMEOUT, DB_READ_RETRIES

from agent_world.drama_demo.features.f06_read_model.display_names import (
    SYSTEM_SENDER_NAME,
    sender_display_name,
)
from agent_world.drama_demo.features.f06_read_model.queries import (
    EventQueriesMixin,
    MessageQueriesMixin,
    MoveQueriesMixin,
    TraceQueriesMixin,
    ...
    def _connect(self) -> sqlite3.Connection:
        if not self.db_path.is_file():
            raise DatabaseReadError(
                "world.db 不存在或未初始化；请重启 run_drama（./agent_world/drama_demo/scripts/start_demo.sh）"
            )
