...
证明：DRAMA_FREE_MOVE 默认关时 DramaActionDispatcher 仍抑制 agent 的 request_move（旧脚本搬人，
...
def test_move_suppressed_when_off() -> None:
    ...
    assert r.get("noop") is True and r.get("reason") == "drama_move_ipc_only", r


def test_move_delegates_when_on() -> None:
    d = DramaActionDispatcher.__new__(DramaActionDispatcher)
    rec = {}
