...
用法：
    python3 -m agent_world.drama_demo.scripts.ops.new_game --premise "一句话剧情概要"
    python3 -m agent_world.drama_demo.scripts.ops.new_game --premise "..." --assets       # 同时出图
    python3 -m agent_world.drama_demo.scripts.ops.new_game --premise "..." --smoke         # 自检后退出(测试)
...
from agent_world.drama_demo.tools.story_studio.naming import slug_story_id as _slug  # noqa: E402
...
def _deepseek_client():
    ...
    if not key:
        raise SystemExit("未配置 DMXAPI_KEY（设计剧情需要）。请在 agent_world/drama_demo/.env 设置 DMXAPI_KEY=...")
...
def main(argv=None) -> int:
    ...
    env.update({
        ...
        "DRAMA_SIM_DIR": str(sim), "FLASK_RUN_PORT": str(args.port),
    ...
    health_path = f"/api/drama/simulations/{story_id}/health"
    ...
    base = f"http://127.0.0.1:{args.port}/api/drama/simulations/{story_id}"
    print("\n" + "=" * 60)
    if ok:
        print(f"  ✅ 世界已就绪！后台世界正在运行。")
        print(f"  · 浏览器开玩前端：cd agent_world/drama_demo/web && npm run dev")
