...
Flask Blueprint (`drama_bp`)：把浏览器请求委托给 L2 features，并提供 Flask 侧 IPC 客户端、
健康探针、WebSocket 注册、错误格式化。注册到 `agent_world.app`（根 `routes.py` 是
转发 shim → `http.routes.drama_bp`）。
...
| 文件 | 作用 |
|------|------|
| `routes.py` | `drama_bp` Blueprint：所有 REST 端点（含大厅 `/lobby/*`），做参数解析 + 委托 L2 + 错误映射（薄路由） |
...
## REST 端点（前缀 `/api/drama/simulations/<sim_id>/`）
...
## 大厅端点（前缀 `/api/drama/lobby/`，不带 `<sim_id>`）
