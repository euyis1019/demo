...
播种代码**零改动**即可从 Story Pack 启动世界（开关式：仅当 HBM_STORY_PACK_SEED=1 时启用）。
这是「换 config 换世界」的运行期落点——seed/build_kernel 拿到的 scenario 来自 Story Pack 而非
hbm_scenario.yaml。等价性由 test_story_pack_scenario_equiv 离线证明（投影 == load_scenario）。
...
def is_story_pack_seed_enabled() -> bool:
    """是否在 Runner 启动时从 Story Pack 播种世界（默认关，旧 hbm_scenario.yaml 路径不变）。
