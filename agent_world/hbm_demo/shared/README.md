...
| `active_game.py` | 运行期「当前在玩哪个故事」的可变状态：`get_active_story`/`set_active_story`（默认 canglan_sword，大厅选/建故事后由 WorldManager 切换）、`active_sim_dir`（当前故事的 sim 目录，优先级：已激活故事 > env `HBM_SIM_DIR` > 默认）——纯状态，不依赖 features（D3） |
