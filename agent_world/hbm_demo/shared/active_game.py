...
默认从 env HBM_STORY_ID（默认 canglan_sword）。前端选/建故事后，由 WorldManager 调
...
def active_sim_dir() -> Path:
    ...
    优先级：
      ...
      2. env HBM_SIM_DIR（Runner 子进程由 WorldManager 显式注入，用来锁定自己那份 sim）；
      3. 默认故事。
    关键点：Flask 进程启动时常会继承到 HBM_SIM_DIR（start_demo.sh / new_game.py 会导出），
    一旦大厅激活了某故事，读路径必须跟着切到 sim/<story>，不能再被旧 env 钉死读到别的世界。
    Runner 子进程从不调 set_active_story（_state['story'] 恒为 None），仍走自己的 HBM_SIM_DIR。
    """
    if _state["story"]:
        return (_HBM_ROOT / "sim" / _state["story"]).resolve()
    env = os.environ.get("HBM_SIM_DIR")
    if env:
        return Path(env).resolve()
    return (_HBM_ROOT / "sim" / get_active_story()).resolve()
