...
log = logging.getLogger("agent_world.hbm_demo.world_manager")
...
def _deepseek_client():
    ...
    if not key:
        raise RuntimeError("未配置 DMXAPI_KEY（设计剧情需要）。请在 agent_world/hbm_demo/.env 设置。")
...
class WorldManager:
    ...
    def activate(self, story_id: str, *, timeout_s: float = 90.0) -> bool:
        ...
        with self._lock:
            ...
            env.update({
                ...
                "HBM_SIM_DIR": str(sim), "PYTHONPATH": str(_ROOT),
            })
    ...
    def _set_active(self, story_id: str) -> None:
        from agent_world.hbm_demo.features.f01_session.paths import clear_scenario_cache
        from agent_world.hbm_demo.shared import active_game, story_config
