...
★让故事**可分支、不死板**（避免一条直线只在最后选结局）：
- 至少有 2 个节点带 2 条或以上出边，代表玩家在关键处的不同选择/行动导向不同后续；
- 多个结局（good/neutral/bad）应由玩家中途的不同选择分别导向，而不是都堆在最后一幕；
- 每一幕的 summary 要制造一个新悬念/冲突/转折，让故事有起承转合，而非每幕重复同一件事。
