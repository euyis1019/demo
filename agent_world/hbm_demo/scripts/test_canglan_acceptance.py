...
HBM = ROOT / "agent_world" / "hbm_demo"
SIM = HBM / "sim" / "_canglan_e2e"
PORT = "5078"
SID = "canglan_sword"
BASE = f"http://127.0.0.1:{PORT}/api/hbm/simulations/{SID}"
    ...
    if envfile.is_file():
        for line in envfile.read_text().splitlines():
            ...
            if line and not line.startswith("#") and "=" in line:
                k, v = line.split("=", 1)
                env[k] = v
    # 不设 HBM_STORY_ID/SEED/ROUTING——验证 canglan 即为默认数据驱动游戏。
    env.update({
        "HBM_SIM_DIR": str(SIM), "FLASK_RUN_PORT": PORT,
