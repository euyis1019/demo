...
#   玩法：当面说页给同地点街坊「送心意🎁/搭把手💪/陪一会儿🍵」；NPC 会可见地干活/歇脚（活动动画）；
#         HUD 看时辰/街坊去向(含好友·心结)/人情温度/街坊传闻；好感跨档/结为好友弹里程碑、
#         每跨时段弹「小镇纪事」；把三人都焐到「挚友」触发涌现式接风
...
- [x] **《焐心小镇》玩法层** 社交人情养成（5 支柱，详见 [玩法设计](../docs/焐心小镇-玩法设计.md)）：
  ...
  真实 LLM 端到端验收 ✓（送礼自主领情升温、口碑在 NPC 间流动、纪事/弹窗截图 ✓）。
- [x] **扩充 agent 行为** 活动动画（NPC 可见地干活/歇脚，复用 KayKit 76 动画）、陪伴互动
  （陪喝一杯/听唠叨/蹲田埂）、好友/心结（由好感自动派生的羁绊里程碑）。**实测结论**：
  deepseek-v4-flash 不会自主调用「结好友/建群/八卦」等结构类工具，已改系统派生 / 沿用涌现口碑
  并删掉不触发的死工具（守「不留无用代码」）。pytest 50 ✓、真实 LLM 回归 ✓（私信必回不被稀释）。
